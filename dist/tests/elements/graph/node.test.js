...
describe('Node', function () {
    describe('constructor', function () {
        it('should create a node with a unique id and add it to the element controller', function () {
            let node = new node_js_1.default(100, 100, 100, 100, 'testNode');
            // chai.expect(circle.id).to.equal(`circle-0`);
            chai.expect(element_js_1.default.controller.get(node.id)).to.equal(node);
    ...
    describe('getters/setters', function () {
        it('should get cx', function () {
            let cx = 50;
            let node = new node_js_1.default(cx, 100, 100, 100, 'testNode');
            chai.expect(node.cx).to.equal(cx);
        });
        it('should get cy', function () {
            let cy = 50;
            let node = new node_js_1.default(100, cy, 100, 100, 'testNode');
            chai.expect(node.cy).to.equal(cy);
        });
        it('should get rx', function () {
            let rx = 50;
            let node = new node_js_1.default(100, 100, rx, 100, 'testNode');
            chai.expect(node.rx).to.equal(rx);
        });
        it('should get ry', function () {
            let ry = 50;
            let node = new node_js_1.default(100, 100, 100, ry, 'testNode');
            chai.expect(node.ry).to.equal(ry);
        });
        it('should get label', function () {
            let label = 'testing';
            let node = new node_js_1.default(100, 100, 100, 100, label);
