...
describe('Circle', function () {
    describe('constructor', function () {
        it('should create a circle with a unique id and add it to the element controller', function () {
            let circle = new circle_js_1.default(100, 100, 100);
            // chai.expect(circle.id).to.equal(`circle-0`);
            chai.expect(element_js_1.default.controller.get(circle.id)).to.equal(circle);
            // chai.expect(Element.count).to.equal(1);
        });
        it('should create elements with successive identifiers', function () {
            let circle1 = new circle_js_1.default(100, 100, 100);
            let circle2 = new circle_js_1.default(100, 100, 100);
            // chai.expect(circle1.id).to.equal(`circle-1`);
            // chai.expect(circle2.id).to.equal(`circle-2`);
            chai.expect(element_js_1.default.controller.get(circle1.id)).to.equal(circle1);
            chai.expect(element_js_1.default.controller.get(circle2.id)).to.equal(circle2);
        });
    });
    ...
    describe('getters/setters', function () {
        it('should get radius', function () {
            let radius = 100;
            let circle = new circle_js_1.default(100, 100, radius);
        ...
        it('should get cx', function () {
            let cx = 75;
            let circle = new circle_js_1.default(cx, 100, 100);
        ...
        it('should get cy', function () {
            let cy = 50;
            let circle = new circle_js_1.default(100, cy, 100);
